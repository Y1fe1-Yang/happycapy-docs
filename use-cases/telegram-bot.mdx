---
title: "From idea to open source — building a 24/7 Telegram Bot skill"
description: "How a Happycapy user built an open-source Telegram Bot skill with 4-layer fault tolerance, AI conversations, and one-command deployment"
---

<img src="/images/use-cases/telegram-bot-repo.png" alt="Telegram Bot Skill GitHub repository" style={{ borderRadius: '12px', marginBottom: '24px' }} />

## Background

A Happycapy user was inspired by another user's Discord Bot article. She decided to build a similar solution for Telegram — but went further by engineering a 4-layer fault tolerance system and open-sourcing everything as an installable Happycapy skill.

> "The predecessor of this project is 'Building a 24/7 Discord Bot in Happycapy Sandbox'. The Telegram Bot began by referencing that structure and evolved its own architecture."

## What she built

### A 24/7 Telegram Bot Skill

An open-source, one-command-setup Telegram bot that runs on Happycapy's sandbox with AI-powered conversations, image analysis, and persistent memory.

<Card
  title="View on GitHub"
  icon="github"
  href="https://github.com/Lucky-Dog07/telegram-bot-skill"
>
  telegram-bot-skill — 7x24 AI-powered Telegram Bot for Happycapy
</Card>

### Key technical difference from Discord

While Discord bots rely on WebSocket connections (requiring discord.js with ~70+ dependencies), this Telegram bot uses **HTTP Long Polling** — native `fetch` API with zero npm dependencies. This makes it lighter and more sandbox-friendly.

### 4-layer fault tolerance architecture

The bot features a robust 4-layer recovery system, evolved from the original 3-layer design:

<Steps>
  <Step title="Auto-Recovery Boot Layer">
    Uses `.zshenv` to trigger `start-bot.sh` on every sandbox session start — because `.bashrc` doesn't work in Happycapy's zsh environment
  </Step>

  <Step title="Layer 1: Supervisord">
    Process manager with dynamic config injection — since `/etc/supervisord.conf` gets reset on sandbox reboot, the script regenerates it every time
  </Step>

  <Step title="Layer 2: Watchdog">
    Custom health monitoring that checks bot status and restarts if needed
  </Step>

  <Step title="Layer 3: Bot Engine">
    The Telegram bot itself with AI Gateway integration for Claude-powered conversations
  </Step>
</Steps>

Plus two independent systems:
- **Monitor** (`monitor.sh`): Health checks every 10 minutes
- **Auto-sync**: Git push every 30 minutes for conversation backup

### Features

<CardGroup cols={2}>
  <Card title="AI Conversations" icon="robot">
    Claude-powered chat through Happycapy's AI Gateway — supports multiple models
  </Card>

  <Card title="Image Analysis" icon="image">
    Multimodal capabilities — send an image and get AI-powered analysis
  </Card>

  <Card title="Persistent Memory" icon="database">
    Per-chat conversation history that survives sandbox restarts via Git sync
  </Card>

  <Card title="One-Command Setup" icon="terminal">
    Single bash command to deploy: token, project dir, owner chat ID, and model selection
  </Card>
</CardGroup>

### Real engineering challenges solved

The article documents the real debugging process — not just the final solution:

- **`.bashrc` doesn't work**: Happycapy uses zsh, not bash — had to switch to `.zshenv`
- **Supervisord config resets**: `/etc/supervisord.conf` is not persistent across reboots — solved with dynamic config generation
- **Long polling conflicts**: Telegram rejects multiple polling connections — required careful process management
- **No cron available**: Had to implement custom scheduled tasks using shell loops
- **Path issues**: `PROJECT_ROOT` resolution failures — fixed with absolute path injection

## An honest take

What makes this write-up stand out is its honesty. The author openly states that true 24/7 uptime hasn't been fully achieved — the bot only auto-restarts when a user opens a Happycapy session, not independently. This kind of transparency is valuable for the community.

> "Honestly, 7x24 is not yet fully achieved. But the architecture is solid and the auto-recovery works reliably within session scope."

## Key takeaways

<CardGroup cols={2}>
  <Card title="Open Source First" icon="code-branch">
    Built as an installable Happycapy skill — anyone can deploy their own Telegram bot with one command
  </Card>

  <Card title="Zero Dependencies" icon="feather">
    HTTP Long Polling with native fetch — no npm packages needed for Telegram API
  </Card>

  <Card title="Standing on Shoulders" icon="users">
    Built on another user's Discord Bot work and evolved the architecture further — community knowledge compounds
  </Card>

  <Card title="Transparent Engineering" icon="magnifying-glass">
    Documents failures and limitations honestly — real debugging stories, not just polished results
  </Card>
</CardGroup>

<Tip>
This project shows the power of Happycapy's community: one user's blog post about a Discord Bot inspired another user to build an improved, open-source version for a different platform. This is how community knowledge compounds — each contribution raises the bar for the next.
</Tip>
